...
class BATLedgerContext {
 public:
  ...
  auto StartTask(Args&&... args) {
    T* instance = new T(this);
    size_t key = std::hash<T*>()(instance);
    tasks_[key] = std::unique_ptr<T>(instance);
    auto result = instance->Start(std::forward<Args>(args)...);
 ...
 private:
  template <typename T>
  void OnTaskFinished(size_t task_key, const T& value) {
